...
class DescribeBatchOperations:
    ...
    def it_distinguishes_run_configs_by_cache_key(self, tmp_path: Path) -> None:
        with IncrementalCache(tmp_path / 'cache') as cache:
            tests = {'t1': 'h1'}
            key_short = cache._build_cache_key('g1', 'src', tests, run_config='timeout=1;lightweight_runner=True')
            key_long = cache._build_cache_key('g1', 'src', tests, run_config='timeout=120;lightweight_runner=True')
            key_off = cache._build_cache_key('g1', 'src', tests, run_config='timeout=1;lightweight_runner=False')

        assert len({key_short, key_long, key_off}) == 3

    def it_misses_a_result_cached_under_another_run_config(self, tmp_path: Path) -> None:
        with IncrementalCache(tmp_path / 'cache') as cache:
            cache.cache_result('g1', 'src', {'t1': 'h1'}, {'status': 'timeout'}, run_config='timeout=1')

            assert cache.get_cached_result('g1', 'src', {'t1': 'h1'}, run_config='timeout=1') == {'status': 'timeout'}
            assert cache.get_cached_result('g1', 'src', {'t1': 'h1'}, run_config='timeout=120') is None
